...
class EventError(ValueError):
    """An event or event file violates E3 schema 3."""
...
def byte_stream(seed: int, length: int) -> bytes:
    ...
    prefix = b"deltafs-e3-v3\0" + struct.pack("<Q", seed)
    chunks = []
    remaining = length
    counter = 0
    while remaining:
...
def preimage_seed(case_id: str) -> int:
    digest = hashlib.sha256(b"deltafs-e3-preimage\0" + case_id.encode("ascii")).digest()
...
def _depth_case(workload: int, size_kib: int, ordinal: int,
    ...
    offset = (randomizer.next() % positions) * BLOCK_SIZE
    payload_seed = randomizer.next()
    case_id = f"e3-depth-w{workload:02d}-f{size_kib:03d}-n{ordinal:03d}"
    variants = []
