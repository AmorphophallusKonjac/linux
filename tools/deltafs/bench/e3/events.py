...
"""Deterministic DeltaFS E3 temporal write-latency events."""
...
class EventError(ValueError):
    """An E3 event violates the fixed schema."""
...
def byte_stream(seed: int, length: int) -> bytes:
    ...
    prefix = b"deltafs-e3-v1\0" + struct.pack("<Q", seed & ((1 << 64) - 1))
...
def _sequence_events(preset: str, run: int, sample: int, family: str,
    ...
    pair_id = f"e3-{configuration['sweep']}-r{run:02d}-s{sample:03d}-{family}-"
