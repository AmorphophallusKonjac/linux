...
"""Analyze paired DeltaFS E3 temporal write-latency artifacts."""
...
class AnalysisError(RuntimeError):
    """E3 artifacts are incomplete or inconsistent."""
...
def discover(root: pathlib.Path) -> list[dict[str, Any]]:
    ...
    if len(result) != len(FS_CONFIGS):
        raise AnalysisError("E3 analysis requires exactly four filesystem result directories")
    return result
...
def validate(result_sets: list[dict[str, Any]]) -> list[str]:
    ...
    for item in result_sets:
        ...
        expected_type = {
            ...
            "xfs_reflink": "xfs",
            "f2fs": "f2fs",
        }.get(config) if isinstance(config, str) else None
        if config not in FS_CONFIGS or manifest.get("schema") != SCHEMA or \
                manifest.get("experiment") != events.EXPERIMENT or \
                manifest.get("deltafs_abi_version") != 2 or \
                manifest.get("access_mode") != "reopen" or \
                manifest.get("direct_baseline") != "lower_direct" or \
                manifest.get("fs_type") != expected_type:
        ...
        for row in item["raw"]:
            ...
            if row.get("edit_e2e_ns", 0) <= 0:
                errors.append(f"non-positive latency in {item['directory']}")
            ...
            if row.get("sample_kind") == "lower_direct":
                transition_valid = before == 0 and after == 0
            else:
                transition_valid = isinstance(before, int) and isinstance(after, int) and \
                    before > 0 and after == before + 1
        ...
        for event_id in expected_ids:
            ...
            if sorted(kinds) != sorted(("lower_direct", baseline_kind, numerator)):
...
def paired_direct_overhead(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Compare every DeltaFS path sample with its direct-base sibling."""
    indexed = {(row["fs_config"], row["event_id"], row["sample_kind"]): row
               for row in rows}
    result = []
    deltafs_kinds = ("upper_resident", "first_touch", "steady_after_copyup")
    for row in rows:
        if row["sample_kind"] not in deltafs_kinds:
            continue
        direct = indexed.get((row["fs_config"], row["event_id"], "lower_direct"))
        if direct is None:
            continue
        direct_latency = float(direct["edit_e2e_ns"])
        if direct_latency <= 0:
            continue
        result.append({
            "fs_config": row["fs_config"], "workload_family": row["workload_family"],
            ...
            "event_id": row["event_id"], "sequence_id": row["sequence_id"],
            "sample_kind": row["sample_kind"],
            "latency_ratio": float(row["edit_e2e_ns"]) / direct_latency,
            "overhead_ns": float(row["edit_e2e_ns"]) - direct_latency,
        })
    return result
...
def analyze(root: pathlib.Path) -> list[str]:
    ...
    if rows:
        ...
        direct_overhead = paired_direct_overhead(rows)
        direct_summary = []
        direct_groups = ("fs_config", "workload_family", "sample_kind",
                         "generation", "history_depth")
        for metric, field in (("ratio_to_lower_direct", "latency_ratio"),
                              ("overhead_ns", "overhead_ns")):
            for item in group_summary(
                    direct_overhead, field, direct_groups,
            ) if direct_overhead else []:
                direct_summary.append({"metric": metric, **item})
        ...
        write_tsv(analysis / "deltafs-overhead.tsv",
                  ["fs_config", "workload_family", "sample_kind", "generation", "history_depth", "metric", "n", "mean", "p50", "p95", "p99", "stddev", "ci95_low", "ci95_high"], direct_summary)
        ...
        direct_x = "history_depth" if result_sets[0]["manifest"].get("sweep") == "history" \
            else "generation"
        plot_png(analysis / "latency-vs-lower.png",
                 [item for item in direct_summary
                  if item["metric"] == "ratio_to_lower_direct"], direct_x)
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if errors:
        ...
        return 1
    print("PASS: E3 analysis completed")
    return 0
