...
"""DeltaFS E4 temporal write-latency benchmark runner."""
...
class E4Error(RuntimeError):
    """An E4 environment or execution contract failure."""
...
def command_output(command: list[str]) -> str:
    ...
    if process.returncode:
        raise E4Error(f"command failed: {' '.join(command)}: {process.stderr.strip()}")
...
def find_mount(path: pathlib.Path) -> dict[str, str]:
    ...
    try:
        ...
        raise E4Error("findmnt returned invalid JSON") from exc
...
def validate_environment(backing_arg: pathlib.Path, out_arg: pathlib.Path) -> tuple[pathlib.Path, pathlib.Path, dict[str, str], int]:
    if os.geteuid() != 0:
        raise E4Error("E4 must run as root")
    if not hasattr(__import__("time"), "CLOCK_MONOTONIC_RAW"):
        raise E4Error("CLOCK_MONOTONIC_RAW is unavailable")
    if backing_arg.is_symlink():
        raise E4Error("BACKING_DIR must not be a symlink")
    backing = backing_arg.resolve(strict=True)
    if not backing.is_dir() or any(backing.iterdir()):
        raise E4Error("BACKING_DIR must be an existing empty directory")
    out = canonical_new_path(out_arg)
    if is_relative_to(out, backing):
        raise E4Error("OUT_DIR must not be inside BACKING_DIR")
    if overlay_mounts():
        raise E4Error("refusing to run while an OverlayFS mount exists")
    ...
    if mount["fstype"] not in FILESYSTEM_MAGICS or filesystem_magic(backing) != FILESYSTEM_MAGICS[mount["fstype"]]:
        raise E4Error("BACKING_DIR must be on ext4 or XFS")
    if out.exists() and (not out.is_dir() or any(out.iterdir())):
        raise E4Error("OUT_DIR must not exist or must be empty")
    out.mkdir(mode=0o700, exist_ok=True)
    if out.stat().st_dev == backing.stat().st_dev:
        raise E4Error("OUT_DIR must be on a different device")
    affinity = sorted(os.sched_getaffinity(0))
    if not affinity:
        raise E4Error("no available CPU")
...
def create_tree(sample: pathlib.Path, sequence_events: list[dict[str, Any]], family: str) -> None:
    ...
    for path in sample.rglob("*"):
        if path.stat().st_dev != sample.stat().st_dev:
            raise E4Error(f"sample crosses backing superblock: {path}")
...
def mount_sample(sample: pathlib.Path, logs: Logs) -> None:
    ...
    if mount["fstype"] != "overlay":
        raise E4Error("sample mount is not OverlayFS")
    ...
    if found:
        raise E4Error(f"mounted OverlayFS feature mismatch: {found}")
...
def read_helper(path: pathlib.Path) -> dict[str, Any]:
    ...
    if set(value) != {"schema", "open_ns", "pwrite_only_ns", "fsync_ns", "close_ns",
        ...
        raise E4Error("helper result schema mismatch")
    return value
...
def verify_image(event: dict[str, Any], path: pathlib.Path, expected: str) -> None:
    if path.stat().st_size != event["file_size"] or sha256_file(path) != expected:
        raise E4Error(f"image oracle failed: {path}")


def sequence_key(values: list[dict[str, Any]]) -> str:
    keys = {event["sequence_id"] for event in values}
    if len(keys) != 1:
        raise E4Error("sequence event set is not singular")
...
def run_one(sample: pathlib.Path, event: dict[str, Any], sample_kind: str,
            ...
            held_fd: int = -1) -> dict[str, Any]:
    target = sample / "merged" / event["relative_path"]
    result_path = sample / "helper-result.json"
    fd = -1
    if access_mode == "held_fd":
        fd = held_fd
        if fd < 0:
            raise E4Error("held_fd mode requires a pre-checkpoint descriptor")
    command = [str(helper), access_mode, str(sample / "merged"), str(target),
    ...
    process = logs.subprocess(command, check=False, pass_fds=(fd,) if fd >= 0 else ())
    ...
    if process.returncode or result["status"] != "ok":
        raise E4Error(f"temporal helper returned {result['status']}")
    return row
    ...
    for event in unique.values():
        ...
        with target.open("r+b", buffering=0) as stream:
            ...
            if len(block) != events.BLOCK_SIZE:
                raise E4Error("resident prewrite could not read one block")
            stream.seek(0)
            if stream.write(block) != len(block):
                raise E4Error("resident prewrite was short")
            os.fsync(stream.fileno())
        verify_image(event, target, event["expected_before_sha256"])
...
def make_sequence_rows(sequence: list[dict[str, Any]], sample_kind: str,
    ...
    try:
        ...
        for measured_generation in range(1, max_generation + 1):
            ...
            if access_mode == "held_fd" and not current_fd:
                for event in generation_events:
                    path = str(sample / "merged" / event["relative_path"])
                    if path not in held_fds:
                        held_fds[path] = os.open(path, os.O_RDWR | os.O_CLOEXEC | os.O_NOFOLLOW)
            ...
            if access_mode == "held_fd" and current_fd:
                for event in generation_events:
                    path = str(sample / "merged" / event["relative_path"])
                    if path not in held_fds:
                        held_fds[path] = os.open(path, os.O_RDWR | os.O_CLOEXEC | os.O_NOFOLLOW)
...
def new_dmesg(before: str, after: str) -> str:
    ...
    raise E4Error("dmesg ring changed without overlap")
...
def run_benchmark(preset: str, backing: pathlib.Path, out: pathlib.Path,
    ...
    if not helper.is_file() or not checkpoint_helper.is_file():
        logs.close()
        raise E4Error("E4 binaries are not built; run make -C tools/deltafs e4-bench")
    ...
    try:
        ...
        for index, (sequence_id, sequence) in enumerate(grouped.items(), 1):
            logs.info(f"E4: sequence {index}/{len(grouped)} {sequence_id}")
            sample_root = work_root / sequence_id
            try:
                variants = [("first_touch", False, False)]
                if configuration["access_mode"] == "reopen":
                    variants.append(("upper_resident", True, False))
                else:
                    variants.append(("current_fd", True, True))
                for variant, resident, current_fd in variants:
                    ...
                    sample_rows = make_sequence_rows(
                        ...
                        logs, configuration["access_mode"], resident, current_fd,
                        setup_path,
                    )
        ...
        if summary["passed"]:
            logs.info(f"PASS: E4 {preset}; sequences={len(grouped)} rows={len(rows)}")
            return 0
        return 1
    finally:
        logs.close()
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Measure DeltaFS E4 temporal write latency")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (E4Error, OSError, events.EventError) as exc:
        print(f"FAIL: E4 runner: {exc}", file=sys.stderr)
        return 1
