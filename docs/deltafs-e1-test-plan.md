# DeltaFS v2 E1 switch bench 设计方案

> 当前实现使用 DeltaFS v2 checkpoint/restore UAPI，工件 schema 为 2。

> E1 只测 DeltaFS checkpoint/restore ioctl 的延迟和正确性。
...
E1 使用一个固定的空 upper workload。每层只放一个确定性 marker，用于验证层顺序，
不做目录数据复制。需要研究 controller 端到端开销或 cache/workload sensitivity 时，
应另建 benchmark，不能继续扩展 E1 的公开接口。
...
E1 只有两个运行命令和一个分析命令：
    ...
    python3 tools/deltafs/bench/e1/analyze.py OUT_DIR
...
E1 不依赖 E2 的 event、FIEMAP、block-stat 或分析代码。两者可以共享 UAPI 头文件，
...
实现后在仓库根目录执行：
    ...
    make -C tools/deltafs e1-bench
    make -C tools/deltafs/bench/e1 check
    python3 -m unittest discover tools/deltafs/bench/e1/tests
...
`e1-bench` 构建独立的 `switch_once`，不调用 `deltafsctl`，也不把 controller wall
time 混入 ioctl latency。`check` 只运行 host-safe 的布局和 parser/analyzer 单元测试。

2026-08-15 当前工作树已完成上述静态门禁：v2 userspace 工具和 E1 helper 以
...
数据；E1 smoke/run 的功能与性能结论仍必须由下述 QEMU/KVM 步骤产生。
...
在仓库根目录执行：
    ...
    if grep -q '^CONFIG_FUNCTION_ERROR_INJECTION=y' .config; then
      make -C tools/deltafs check-e0-checkpoints CHECKPOINT_MODE=enabled
    else
      make -C tools/deltafs check-e0-checkpoints CHECKPOINT_MODE=disabled
    fi

期望生成 `arch/x86/boot/bzImage`、`fs/overlayfs/overlay.ko`、v2 controller/layout/ioctl
工具以及 `tools/deltafs/bench/e1/switch_once`。
...
为当前 backing filesystem 准备空目录：
    ...
    rm -rf /mnt/deltafs-test/disk1/e1-backing \
      /mnt/deltafs-test/results/e1-smoke \
      /mnt/deltafs-test/results/e1-run
    mkdir -p /mnt/deltafs-test/disk1/e1-backing

先运行 smoke：

    python3 tools/deltafs/bench/e1/run.py smoke \
      /mnt/deltafs-test/disk1/e1-backing \
      /mnt/deltafs-test/results/e1-smoke
...
smoke 通过后，重新创建空 backing 目录并运行固定主实验：
    ...
    python3 tools/deltafs/bench/e1/analyze.py \
      /mnt/deltafs-test/results/e1-run
...
`PASS: E1 analysis completed; groups=16`，且分析目录包含第 7.3 节的五个固定文件。对
`ext4_noreflink`、`xfs_noreflink` 和 `xfs_reflink` 分别使用独立 image 重复上述命令，
不要在同一个已挂载 image 上重新格式化。

### 9.5 日志和失败诊断

失败时不要删除 `BACKING_DIR/.e1-work` 或 `OUT_DIR`。执行：

    R=/mnt/deltafs-test/results/e1-run
...
controller 初始化命令执行完整功能矩阵：
    ...
    make -C tools/deltafs check-e0-fast-path
    tools/deltafs/deltafs_e0_phase1_test.sh \
    ...
    tools/deltafs/deltafs_e0_checkpoint_callsite_test.sh \
      fs/overlayfs/deltafs.o fs/overlayfs/deltafs.c auto

预期 phase-1、acceptance 和 callsite 脚本均以 `PASS` 结束；再运行 E1 smoke/run
