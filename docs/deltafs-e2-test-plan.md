# DeltaFS E2 cache-neutral copy-up benchmark detailed design

> E2 measures one copy-up edit after a native DeltaFS v2 checkpoint. It is
> independent of the E1 ioctl-latency benchmark and does not use `deltafsctl`.
...
E2 compares four backing filesystem configurations:

- `ext4`;
- `xfs_noreflink` (`mkfs.xfs -m reflink=0`);
- `xfs_reflink` (`mkfs.xfs -m reflink=1`).
- `f2fs` (`mkfs.f2fs`); F2FS has no reflink mode, so no `noreflink` suffix is used.

E2 contains two fixed experiments. `copyup_matrix` retains the original
...
E2 deliberately follows E1's fixed-preset interface. Users run only:

    sudo python3 tools/deltafs/bench/e2/run.py \
...
`BACKING_DIR` is an empty dedicated directory on ext4, XFS, or F2FS and may be a
...
- recognizes ext4/XFS/F2FS and obtains XFS `reflink=0/1` by passing the `findmnt`
...
analysis. E2 does not import E1 code or artifacts.
...
line:

    {
      ...
      "event_id": "e2-w01-f004-d01-n000",
      "case_id": "e2-w01-f004-d01-n000",
      "workload": 1,
...
The byte stream is defined as concatenated SHA-256 digests of:
    ...
    "deltafs-e2-v3\0" || little_endian_u64(seed) || little_endian_u64(counter)

The preimage seed is the low little-endian 64 bits of
`SHA256("deltafs-e2-preimage\0" || case_id)`. Depth variants share one
...
hashes across all four filesystems before pairing by `event_id`.
...
example an 8-block in-place write cannot fit in a 4-KiB file. E2 v2 treats the
...
Cache state is not an E2 variable. The helper does not issue
...
terminate with exactly one final `LAST`. E2 v1 understands only `LAST`,
...
OverlayFS E2 artifacts with current results.
...
`analyze.py RESULTS_ROOT` recursively discovers E2 manifests outside its own
...
four filesystems must share one event hash and complete edit/control schedules.

It first performs an exact `event_id` join across all four configurations.
...
`xfs_metadata` is `ext4 - xfs_noreflink`; `reflink` is
...
Passing requires every experiment-specific planned cell at its preset count, all four
...
The E2 Makefile builds with `-Wall -Wextra -Werror`; `check` runs only C fixture
...
The host gate covers v2 userspace tools and E2. E1 also uses the v2 ABI, but its
latency benchmark is independent and is not an E2 prerequisite. No module is
...
From the repository root:
    ...
    make -C tools/deltafs clean e0-tools e2-bench
...
Expected files include:
    ...
    tools/deltafs/bench/e2/checkpoint_v2
    tools/deltafs/bench/e2/copyup_bench

### 11.2 Prepare dedicated images and boot

Use four disposable data images so filesystem results have isolated counters.
The following destroys only the explicitly named new image files:

    truncate -s 20G /absolute/path/e2-ext4.raw
    truncate -s 20G /absolute/path/e2-xfs-noreflink.raw
    truncate -s 20G /absolute/path/e2-xfs-reflink.raw
    truncate -s 20G /absolute/path/e2-f2fs.raw
    mkfs.ext4 -F /absolute/path/e2-ext4.raw
    mkfs.xfs -f -m reflink=0 /absolute/path/e2-xfs-noreflink.raw
    mkfs.xfs -f -m reflink=1 /absolute/path/e2-xfs-reflink.raw
    mkfs.f2fs -f /absolute/path/e2-f2fs.raw

Boot with the worktree kernel and the four images:
    ...
    qemu-system-x86_64 \
      ...
      -drive if=virtio,format=raw,file=/absolute/path/e2-ext4.raw \
      -drive if=virtio,format=raw,file=/absolute/path/e2-xfs-noreflink.raw \
      -drive if=virtio,format=raw,file=/absolute/path/e2-xfs-reflink.raw \
      -drive if=virtio,format=raw,file=/absolute/path/e2-f2fs.raw \
      -virtfs local,path="$REPO",mount_tag=host,security_model=none

The examples below assume the guest names the data disks `/dev/vdb` through
`/dev/vde`; verify with `lsblk -f` before mounting.

### 11.3 Guest setup and E0 acceptance gate
    ...
    mkdir -p /mnt/e2/{ext4,xfs-noreflink,xfs-reflink,f2fs,results}
    mount /dev/vdb /mnt/e2/ext4
    mount /dev/vdc /mnt/e2/xfs-noreflink
    mount /dev/vdd /mnt/e2/xfs-reflink
    mount /dev/vde /mnt/e2/f2fs

    xfs_info /mnt/e2/xfs-noreflink | grep -w 'reflink=0'
    xfs_info /mnt/e2/xfs-reflink   | grep -w 'reflink=1'
    for M in /mnt/e2/ext4 /mnt/e2/xfs-noreflink /mnt/e2/xfs-reflink; do
      findmnt -no TARGET,SOURCE,FSTYPE "$M"
    done
    findmnt -no TARGET,SOURCE,FSTYPE /mnt/e2/f2fs
    ...
    modprobe overlay
    cd /mnt/host
    make -C tools/deltafs e0-tools e2-bench

Use two dedicated roots on different filesystems to run the v2 gate before E2:

    make -C tools/deltafs test-e0-controller
    mkdir -p /mnt/e2/xfs-reflink/v2-main /mnt/e2/xfs-noreflink/v2-extra
    tools/deltafs/deltafs_e0_acceptance_test.sh \
      --backing-root /mnt/e2/xfs-reflink/v2-main \
      --extra-backing-root /mnt/e2/xfs-noreflink/v2-extra
...
E2 after a skipped or failed E0 acceptance run. The acceptance cleanup unloads
the module, so reload the installed worktree module before E2:
...
backing filesystem:
    ...
    mkdir /mnt/e2/ext4/e2-smoke /mnt/e2/xfs-noreflink/e2-smoke \
      /mnt/e2/xfs-reflink/e2-smoke /mnt/e2/f2fs/e2-smoke
    mkdir -p /var/tmp/e2-results/smoke

    python3 tools/deltafs/bench/e2/run.py smoke \
      /mnt/e2/ext4/e2-smoke /sys/block/vdb/stat \
      /var/tmp/e2-results/smoke/ext4
    python3 tools/deltafs/bench/e2/run.py smoke \
      /mnt/e2/xfs-noreflink/e2-smoke /sys/block/vdc/stat \
      /var/tmp/e2-results/smoke/xfs_noreflink
    python3 tools/deltafs/bench/e2/run.py smoke \
      /mnt/e2/xfs-reflink/e2-smoke /sys/block/vdd/stat \
      /var/tmp/e2-results/smoke/xfs_reflink
    python3 tools/deltafs/bench/e2/run.py smoke \
      /mnt/e2/f2fs/e2-smoke /sys/block/vde/stat \
      /var/tmp/e2-results/smoke/f2fs
...
every edit and control, end with `PASS: E2 smoke ... invalid=0 failed=0`, and
analysis must end with `PASS: E2 analysis completed`. There are 36 edit events
...
file:
    ...
    mkdir -p /var/tmp/e2-results/run

    run_e3() {
      FS=$1
      case "$FS" in
        ext4)          BACKING=/mnt/e2/ext4 STAT=/sys/block/vdb/stat ;;
        xfs_noreflink) BACKING=/mnt/e2/xfs-noreflink STAT=/sys/block/vdc/stat ;;
        xfs_reflink)   BACKING=/mnt/e2/xfs-reflink STAT=/sys/block/vdd/stat ;;
        f2fs)          BACKING=/mnt/e2/f2fs STAT=/sys/block/vde/stat ;;
        *) return 2 ;;
      esac
      WORK="$BACKING/e2-run"
      OUT="/var/tmp/e2-results/run/$FS"
      mkdir "$WORK"
      python3 tools/deltafs/bench/e2/run.py run "$WORK" "$STAT" "$OUT"
    }
    ...
    python3 tools/deltafs/bench/e2/analyze.py /var/tmp/e2-results/run
...
all four filesystem outputs, all six bins, 18 complete cells, zero
...
Use new empty backing directories and separate result roots:
    ...
    mkdir -p /var/tmp/e2-results/depth-smoke /var/tmp/e2-results/depth-run
    ...
    run_depth_e3() {
      ...
      WORK="$BACKING/e2-$PRESET"
      mkdir "$WORK"
      python3 tools/deltafs/bench/e2/run.py "$PRESET" "$WORK" "$STAT" "$ROOT/$FS"
    }

    for FS in ext4 xfs_noreflink xfs_reflink f2fs; do
      run_depth_e3 depth-smoke "$FS" /var/tmp/e2-results/depth-smoke
    done
    python3 tools/deltafs/bench/e2/analyze.py /var/tmp/e2-results/depth-smoke
...
After smoke passes, reboot or use new disposable filesystems/directories, then:
    ...
    for FS in ext4 xfs_noreflink xfs_reflink f2fs; do
      run_depth_e3 depth-run "$FS" /var/tmp/e2-results/depth-run
    done
    python3 tools/deltafs/bench/e2/analyze.py /var/tmp/e2-results/depth-run
...
On any failure, do not delete `.e2-work`, result JSONL, or FIEMAP dumps. Replace
`R` and `DEV` with the failing output and measured device:

    R=/var/tmp/e2-results/run/xfs_reflink
